...
def wikipedia_scrape(topic, links_count=10, verbose=True):
    ...
    progress = (
        tqdm(desc="Collecting Data", unit="", total=len(page_links))
        if verbose
        else None
    )

    # concurrently downloading pages from reference link
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        future_link = {
            executor.submit(donwload_reference_link, link): link for link in page_links
        }
    ...
    dataframe["categories"] = dataframe.categories.apply(
        lambda x: [item.replace("Category:", "") for item in x]
    )
    dataframe["topic"] = topic
    tqdm.pandas(desc="sentence counting:")
    dataframe["sentence_count"] = dataframe["content"].progress_apply(
        lambda x: len(x.split("."))
    )
    dataframe.sort_values(
        by=["sentence_count"], ascending=False, inplace=True, ignore_index=True
    )
    return dataframe
